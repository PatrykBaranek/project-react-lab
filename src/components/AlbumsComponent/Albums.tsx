...
export interface IAlbumsProps {
  ...
  setSearchParams: ({ albumId, username, albumTitle }: ISearchAlbumParams) => void;
}
...
export const Albums: FC<IAlbumsProps> = ({ albums, setSearchParams }) => {
  const { data: users } = useFetch<IUser[]>('https://jsonplaceholder.typicode.com/users');

  return (
    <div className="albums-container">
      {albums &&
        albums.map((album) => (
          <div key={album.id} className={`album ${albums.length === 1 ? 'single-album' : ''}`}>
